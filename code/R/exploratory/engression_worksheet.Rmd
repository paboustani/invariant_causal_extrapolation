...
polygon(c(rev(X_range), X_range), 
        c(rev(Yhat_quantile[,2]), Yhat_quantile[,1]), 
        col = rgb(1, 0, 0, alpha = 0.4), 
        border = NA
      )
...
# Plot the original data points
points(X, Y, col = rgb(0, 0, 0, alpha = 0.4), pch = 16)


# add mean prediction
lines(X_range, Yhat_eng, col = rgb(1, 0, 0, alpha = 0.7), lwd=3)

# add median prediction
lines(X_range, Yhat_median, col = rgb(0, 1, 0, alpha = 0.7), lwd=3)
```
