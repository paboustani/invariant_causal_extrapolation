...
directory <- getwd()
method <- "GAM"
title <- paste("simulation/SimulationReport", method, sep = "_")
impdir = directory
fileform = "csv"
delim = ";"
...
```{r}
statsMat <- matrix(NA, 2,2)
statsMat[2,1] <- mean(Jaccard, na.rm = TRUE)
statsMat[1,2] <- mean(1-intInParents)
```
...
fig_name <- "JACCARD_FWER_GAMeng"

postscript(paste(figure_dir, fig_name, ".eps", sep = ""),
           width = 6, height = 6, horizontal = FALSE)  # Open EPS device

plot(x, y, xlim = x_range, ylim = y_range, type = "n", xlab = "", ylab = "", xaxt = "n", yaxt = "n", bty = "n", ann = FALSE)

# Add axes
axis(1, at = seq(0, 0.3, by = 0.05), labels = sprintf("%.2f", seq(0, 0.3, by = 0.05)))
...
# Plot the first observation as a filled pink square
points(x[1], y[1], pch = 22, col = "pink", bg = "pink", cex = 2)

# Plot the second observation as a green star
points(x[2], y[2], pch = 2, col = "green", cex = 1.5, lwd=3.4)

# Add legend
legend("right",
       legend = c("GAM", "Engression"),
       pch = c(22, 2), col = c("pink", "green"),
       pt.bg = c("pink", "green"), pt.cex = 1,
       cex = 0.8, bty = "n", title = "method")
...
target <- "Z_3"
quant <- 0.4
regressors <- "parents" # all, parents, descendants, ancestors
...
# engLossMat <- numeric(d_gran)
# names(engLossMat) <- paste("D",1:length(engLossMat),sep = "")
# 
# gamLossMat <- numeric(d_gran)
# names(gamLossMat) <- paste("D",1:length(gamLossMat),sep = "")
...
# for (rep in 1:Reps){
      ...
      engLoss <- numeric(d_gran)
      gamLoss <- numeric(d_gran)
      
      for (i in 1:d_gran) {
        d_subset <- which(distTest<=d_seq[i])
        
        # MSE 
        engLoss[i] <- round(mean((Yhat_eng[d_subset] - testData[d_subset,target])^2), 3)
        gamLoss[i] <- round(mean((Yhat_gam$predictions[d_subset] - testData[d_subset,target])^2), 3)
        
        # ABS
        # engLoss[i] <- round( max( abs(Yhat_eng[d_subset] - testData[d_subset,target]) ), 3)
        # gamLoss[i] <- round( max( abs(Yhat_gam$predictions[d_subset] - testData[d_subset,target])), 3)
        # engLoss[i] <- round(chebyshev(Yhat_eng[d_subset], testData[d_subset,target]),3)
        # gamLoss[i] <- round(chebyshev(Yhat_gam$predictions[d_subset], testData[d_subset,target]),3)
      }
      
      names(engLoss) <- paste("D",1:length(engLoss),sep = "")
      names(gamLoss) <- paste("D",1:length(gamLoss),sep = "")
      
      # engLossMat <- rbind(engLossMat, engLoss)
      # gamLossMat <- rbind(gamLossMat, gamLoss)
      ...
      record_stats( c(stats, engLoss),
                 title = paste("ExtraplolationStorage", "eng", sep = "_") )

      record_stats( c(stats, gamLoss),
...
delim <- ";"
MSEengMat <- read.csv(paste("ExtraplolationStorage", "eng_F1.csv", sep = "_"), 
                       header = TRUE, sep = delim, encoding="UTF-8" )
MSEengMat <- MSEengMat[, paste("D",1:d_gran, sep = "")] # quant == 0.4
MSEengMat[] <- lapply(MSEengMat, as.numeric)
MSEengMat <- as.matrix( MSEengMat )
# 
# MSEengMat[] <- lapply(MSEengMat, function(x) if(is.integer(x)) as.numeric(x) else x)
# MSEengMat <- as.matrix( MSEengMat[,paste("D",1:d_gran, sep = "")] )


MSEgamMat <- read.csv(paste("ExtraplolationStorage", "GAM_F1.csv", sep = "_"), 
                       header = TRUE, sep = delim, encoding="UTF-8", fill = TRUE)
MSEgamMat <- MSEgamMat[,paste("D",1:d_gran, sep = "")] # quant == 0.4
MSEgamMat[] <- lapply(MSEgamMat, as.numeric)
MSEgamMat <- as.matrix( MSEgamMat)

engMean <- colMeans(MSEengMat, na.rm = TRUE)
GAMmean <- colMeans(MSEgamMat, na.rm = TRUE)

plot(d_seq, GAMmean, type = "l", 
     ylim = c(0, range(engMean,GAMmean)[2] ) )
points(d_seq, engMean, col = "red", type = "l")
```



```{r}
fig_name <- "ExtrapolationUncertainty_GAMeng"

postscript(paste(figure_dir, fig_name, ".eps", sep = ""),
           width = 6, height = 6, horizontal = FALSE)  # Open EPS device

plot(x = d_seq, y = GAMmean, ylim = c(0, range(engMean,GAMmean)[2] ), type = "l", 
     bty = "n", ann = FALSE, col = "pink",  lwd=2)

points(d_seq, engMean, col = "green", type = "l", lwd=2)

# Add legend
legend("right",
       legend = c("GAM", "Engression"),
       pch = "-", col = c("pink", "green"),
       pt.cex = 2.8,
       cex = 0.8, bty = "n", title = "method")

# Add axis labels 
mtext("d", side = 1, line = 2)
