...
obtain_S <- function(target, d, vars=c() ){
...
g_k <- function(parents, coeff_signs, coeffs, id, multiplic, noise, preSI) {
  ...
  if (!preSI) {
    ...
    beta_valus <- coeff_values * coeffs
    
    # apply coefficients, function f_id
    if (length(beta_valus) == 1){
      parents <- parents * beta_valus
      data_value <- f(parents + noise, id)
    } else {
      parents <- parents %*% diag(beta_valus)
      parents <- rowSums(parents)
      parents <- parents + noise
...
# simulate data (with and without intervention)
sim_data <- function(n, d, dag, coeff_signs, dag_coefs, target, eta_df, f_id, multiplic, 
                     E, shift, interv, epsilon_df, meanshift, strength, verbose = FALSE, preSI = FALSE) {
  ...
  if (E == 1) {
    ...
    for (k in 1:d) {
      ...
      if(length(parents) == 0) {
        ...
        beta_coef <- get_coeff(node = node, coeffs = dag_coefs)
        if (verbose) cat("Parental signs =", signs, "\n")
        
        eta_sample <- gen_eta(n, eta_df)
        X[,k] <- g_k(parents = X[,parents], coeff_signs = signs, coeffs = beta_coef,
                     id = f_id, multiplic = multiplic, noise = eta_sample, preSI = preSI) # change for pra-ANM
...
# simulate data
gen_sample <- function(n, d, dag, signs, betas, target = "random", verbose = FALSE, 
                       preSI = FALSE, monotone_fun = FALSE){
  ...
  if (monotone_fun) {
    id <- sample(c("softplus", "square", "cubic", "log"), 1)
  } else {
    id <- sample(c(1, 2, 3, 4), 1)
  }
  
  # assignment of environments
  n_vec <- sample(1:n, 1)
  ...
  for (E in 1:3){
    ...
    # sample from dag
    out <- sim_data(n_vec[E], d, dag, coeff_signs = signs, dag_coefs = betas, 
                    target, eta_df, f_id = id, 
                    multiplic, E, shift, interv, epsilon_df, meanshift, strength, 
                    verbose = verbose, preSI)
